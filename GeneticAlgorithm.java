...
public Population crossoverPopulation(Population population) {
        ...
        for (int populationIndex = 0; populationIndex < population.size(); populationIndex++) {
            ...
            if (this.rateOfCrossover > ThreadLocalRandom.current().nextDouble() && populationIndex > this.count) {
                Individual offspring = new Individual(parent1.getChromosomeLength());
                Individual parent2 = selectionFunction(population);
                for (int geneIndex = 0; geneIndex < parent1.getChromosomeLength(); geneIndex++) {
                    if (0.5 > ThreadLocalRandom.current().nextDouble()) {
        ...
        for (int populationIndex = 0; populationIndex < population.size(); populationIndex++) {
            ...
            for (int geneIndex = 0; geneIndex < individual.getChromosomeLength(); geneIndex++) {
                if (populationIndex > this.count) {
                    if (this.rateOfMutation > ThreadLocalRandom.current().nextDouble()) {
 ...
 * 
 * 
 * public Population mutatingPopulation(Population population, Schedule
 * schedule) {
 * 
 * Population newPopulation = new Population(this.popSize);
 * 
